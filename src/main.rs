...
pub fn confine_player_movement(
    ...
    if let Ok(mut transform) = player_query.get_single_mut() {
        ...
        let half_payer_size = PLAYER_SIZE / 2.0;
        let x_min = 0.0 + half_payer_size;
        let x_max = window.width() - half_payer_size;
        let y_min = 0.0 + half_payer_size;
        let y_max = window.height() - half_payer_size;
    ...
    if let Ok(player_transform) = player_query.get_single() {
        for (star_entity, star_transform) in star_query.iter() {
            ...
            if distance < player_radius + star_radius {
                println!("Star collected! life++");
                let sound_effect = asset_server.load("audio/impactGlass_heavy_000.ogg");
